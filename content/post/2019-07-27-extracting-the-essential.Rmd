...
Reading large texts takes time. Sometimes it would be useful to extract essential sentences of a text to get a first impression of the whole.

To extract the essential I wil use the following R packages
...
To scrape the data we simply define the relevant URL a node which indicates a corresponding CSS selector. Because we only want the text of our article, we choose `p` as our relevant node. The [p](https://html.com/tags/p/#ixzz5utCwr0gg) element is used to identify blocks of paragraph text.

There is a wide variity of nodes we can define but do not need in this context. For exmaple we cld scrape tables, rating, pictures ...
...
Textrank needs a a dataframe with sentences and a dataframe with words which are part of these sentences.

Here we only take nouns and adjectives to for finding overlaps between sentences.
...
We can now extract the top n most relevant sentences where n defines the number of sentences which shold be mentioned.
